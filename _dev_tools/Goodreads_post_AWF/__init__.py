...
class Goodreads(Source):
    ...
    EDITION_SUFFIX_RE = re.compile(
        r'\s+\([^\)]*(?:Hardcover|Paperback|Kindle Edition|Mass Market Paperback|'
        r'Audio CD|Audiobook|Audible Audio|Portuguese Edition|ebook|eBook|e-book|'
        r'Spanish Edition|French Edition|German Edition|Italian Edition|Dutch Edition|'
        r'edition|Edition)[^\)]*\)\s*$', re.IGNORECASE)
    ...
    def id_from_url(self, url):
        ...
        # Also handle work/editions URLs - extract the work ID
        match = re.match(self.BASE_URL + r"/work/editions/(\d+).*", url)
        if match:
            # For editions pages, we return the work ID as the identifier
            # The worker will extract the actual book ID from the page
            return (self.ID_NAME, match.groups(0)[0])
        return None

    def create_query(self, log, title=None, authors=None, identifiers={}, asin=None):
        # Use the autocomplete API which is not blocked by WAF
        isbn = check_isbn(identifiers.get('isbn', None))
        if isbn:
            return Goodreads.BASE_URL + '/book/auto_complete?format=json&q=' + isbn
        elif asin:
            return Goodreads.BASE_URL + '/book/auto_complete?format=json&q=' + asin
            ...
            q = '+'.join(tokens)
            return Goodreads.BASE_URL + '/book/auto_complete?format=json&q=' + q
        return None
    ...
    def identify(self, log, result_queue, abort, title=None, authors=None,
        ...
        if goodreads_id:
            # Keep /book/show URLs for Goodreads More Tags compatibility.
            # The worker now carries the work ID and falls back to /work/editions/{workId}
            # internally when Goodreads blocks the book page.
            # Try to get work ID via autocomplete API using ISBN or title
            work_id = None
            isbn = check_isbn(identifiers.get('isbn', None))
            query_title = isbn if isbn else title
            if query_title:
                query = Goodreads.BASE_URL + '/book/auto_complete?format=json&q=' + query_title
                try:
                    log.info('Getting work ID via autocomplete for: %s' % query_title)
                    raw = br.open_novisit(query, timeout=timeout).read()
                    json_result = json.loads(raw)
                    if json_result and len(json_result) >= 1:
                        work_id = json_result[0].get('workId')
                        if work_id:
                            log.info('Got work ID: %s for query: %s' % (work_id, query_title))
                except Exception as e:
                    log.exception('Failed to get work ID via autocomplete')
            # When work_id is found via autocomplete, use _parse_autocomplete_results to get description/rating data
            if work_id:
                query = Goodreads.BASE_URL + '/book/auto_complete?format=json&q=' + query_title
                try:
                    log.info('Getting full autocomplete data for: %s' % query_title)
                    raw = br.open_novisit(query, timeout=timeout).read()
                    json_result = json.loads(raw)
                    if json_result and len(json_result) >= 1:
                        book_id = json_result[0].get('bookId')
                        description_html = json_result[0].get('description', {}).get('html', '')
                        avg_rating = json_result[0].get('avgRating')
                        ratings_count = json_result[0].get('ratingsCount')
                        log.info('Got full data: book_id=%s desc_len=%d rating=%s votes=%s' % (
                            book_id, len(description_html) if description_html else 0, avg_rating, ratings_count))
                        matches.append(('%s/book/show/%s' % (Goodreads.BASE_URL, book_id), work_id, description_html, avg_rating, ratings_count))
                    else:
                        matches.append(('%s/book/show/%s' % (Goodreads.BASE_URL, goodreads_id), work_id))
                except Exception as e:
                    log.exception('Failed to get full autocomplete data')
                    matches.append(('%s/book/show/%s' % (Goodreads.BASE_URL, goodreads_id), work_id))
            else:
                matches.append(('%s/book/show/%s' % (Goodreads.BASE_URL, goodreads_id), work_id))
        else:
            # Can't find a valid id, so search using the autocomplete API
            ...
            try:
                ...
                if not raw:
                    ...
                    return
                # Parse JSON response from autocomplete API
                json_results = json.loads(raw)
                self._parse_autocomplete_results(log, title, authors, json_results, matches, timeout)
            except:
                msg = 'Failed to parse goodreads autocomplete response: %r' % query
                log.exception(msg)
                return msg
        ...
        for i, match in enumerate(matches):
            if isinstance(match, tuple):
                if len(match) >= 5:
                    url, work_id, description_html, avg_rating, ratings_count = match[:5]
                elif len(match) == 2:
                    url, work_id = match
                    description_html = avg_rating = ratings_count = None
                else:
                    url, work_id = match, None
                    description_html = avg_rating = ratings_count = None
            else:
                url, work_id = match, None
                description_html = avg_rating = ratings_count = None
            log.info('Creating Worker url=%s work_id=%s desc_len=%d rating=%s votes=%s' % (
                url, work_id,
                len(description_html) if description_html else 0,
                avg_rating,
                ratings_count
            ))
            workers.append(Worker(url, result_queue, br, log, i, self, work_id=work_id,
                                  description_html=description_html, avg_rating=avg_rating, ratings_count=ratings_count))
    ...
    def _parse_autocomplete_results(self, log, orig_title, orig_authors, json_results, matches, timeout):
        '''
        Parse results from the autocomplete API (JSON format).
        This replaces the blocked /search endpoint.
        '''
        if not json_results:
            log.error('No results from autocomplete API')
            return

        title_tokens = list(self.get_title_tokens(orig_title))
        author_tokens = list(self.get_author_tokens(orig_authors))

        def ismatch(title, author_name):
            title = lower(title)
        ...
        for result in json_results:
            ...
            # Get additional data from autocomplete API for fallback
            description_html = result.get('description', {}).get('html', '') if result.get('description') else ''
            avg_rating = result.get('avgRating')
            ratings_count = result.get('ratingsCount')

            if not book_id:
                continue

            if ismatch(title, author_name):
                log.info('Found match: %s by %s' % (title, author_name))
                # Use book/show URL for compatibility with Goodreads More Tags.
                # Worker will internally fall back to /work/editions/{workId} when WAF blocks the book page.
                # Pass autocomplete data (description, rating, votes) for fallback
                matches.append(('%s/book/show/%s' % (Goodreads.BASE_URL, book_id), work_id, description_html, avg_rating, ratings_count))
    ...
    def _parse_editions_for_book(self, log, editions_url, matches, timeout, title_tokens):
        ...
        if 'awsWafCookieDomainList' in raw_utf8 or 'challenge.js' in raw_utf8:
            log.error('Goodreads returned bot protection challenge page - WAF blocking request')
            return
