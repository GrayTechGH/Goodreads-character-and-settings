...
class Worker(Thread): # Get details
    ...
    def __init__(self, url, result_queue, browser, log, relevance, plugin, timeout=20, work_id=None,
                 description_html=None, avg_rating=None, ratings_count=None):
    ...
    def run(self):
        try:
            ...
            while retry and retryCount <= 10:
                ...
                if retry and self.work_id:
                    self.log.info('Falling back to editions page for work ID: %s' % self.work_id)
                    self.url = '%s/work/editions/%s' % (self.plugin.BASE_URL, self.work_id)
                    retry = self.get_details()
        except:
            self.log.exception('get_details failed for url: %r'%self.url)

    def fallback_to_editions_page(self):
        if not self.work_id:
            return False
        self.log.info('Falling back to editions page for work ID: %s' % self.work_id)
        self.url = '%s/work/editions/%s' % (self.plugin.BASE_URL, self.work_id)
    ...
    def get_details(self):
        ...
        # Check for WAF challenge page (AWS bot protection)
        if 'awsWafCookieDomainList' in raw_utf8 or 'challenge.js' in raw_utf8:
            self.log.error('Goodreads returned bot protection challenge page - WAF blocking request')
            return self.fallback_to_editions_page()

        if '<title>404 - ' in raw_utf8:
            self.log.error('URL malformed: %r'%self.url)
            return False

        # Check if this is an editions page (work/editions/...)
        if '/work/editions/' in self.url:
            return self._parse_editions_page(raw_utf8)
        ...
        try:
            ...
            if not book_json:
                self.log.error('NO_JSON_LEGACY_HTML_FALLBACK_TRIGGERED - work_id=%s' % self.work_id)
                # If we have a work_id, fall back to editions page
                return self.fallback_to_editions_page()
    ...
    def _parse_editions_page(self, raw_html):
        '''
        Parse the editions page to extract book details.
        This is used as a fallback when book/show page is blocked by WAF.
        '''
        # Check for redirect response (Goodreads redirects work/editions/ID to work/editions/ID-title)
        if '<body>You are being <a href=' in raw_html:
            # Extract redirect URL and follow it
            redirect_match = re.search(r'href="([^"]+)"', raw_html)
            if redirect_match:
                redirect_url = redirect_match.group(1)
                if redirect_url.startswith('/'):
                    redirect_url = self.plugin.BASE_URL + redirect_url
                self.log.info('Following redirect to: %s' % redirect_url)
                try:
                    raw = self.browser.open_novisit(redirect_url, timeout=self.timeout).read().strip()
                    raw_html = raw.decode('utf-8', errors='replace')
                except Exception as e:
                    self.log.exception('Failed to follow redirect')
                    return False
        ...
        # Check for "Sorry you got bad directions" error page
        if 'Sorry you got bad directions' in raw_html:
            self.log.error('Invalid editions page URL')
            return False

        # Extract work ID from URL (handle both /work/editions/ID and /work/editions/ID-title formats)
        work_id_match = re.search(r'/work/editions/(\d+)', self.url)
        if not work_id_match:
            self.log.error('Could not parse work ID from URL: %r'%self.url)
            return False
        work_id = work_id_match.group(1)

        # Get the first edition (main book)
        edition_divs = root.xpath('//div[@class="editionData"]')
        if not edition_divs:
            self.log.error('No edition data found on editions page')
            return False

        # Parse the first edition
        first_edition = edition_divs[0]

        # Get book title
        title_link = first_edition.xpath('.//a[@class="bookTitle"]')
        title = None
        if title_link:
            title = title_link[0].text.strip() if title_link[0].text else None

        # Get author
        author_link = first_edition.xpath('.//a[@class="authorName"]')
        if not author_link:
            author_link = first_edition.xpath('.//span[@itemprop="author"]//a')
        authors = []
        if author_link:
            author_text = author_link[0].text_content() if hasattr(author_link[0], 'text_content') else author_link[0].text
            if author_text:
                authors = self.normalize_authors([author_text.strip()])

        # Get goodreads ID from the book link
        book_link = first_edition.xpath('.//a[@class="bookTitle"]/@href')
        if book_link:
            book_id_match = re.search(r'/book/show/(\d+)', book_link[0])
            if book_id_match:
                self.goodreads_id = book_id_match.group(1)

        if not title:
            self.log.error('Could not parse title from editions page')
            return False

        if not authors:
            # Try to get author from the page header
            author_from_header = root.xpath('//a[@class="authorName"]/span/text()')
            if author_from_header:
                authors = self.normalize_authors([author_from_header[0].strip()])
            else:
                authors = ['Unknown']

        title = self.strip_edition_suffix(title)
        mi = Metadata(title, authors)
        if self.goodreads_id:
            mi.set_identifier('goodreads', self.goodreads_id)

        # Try to get additional details from the edition data
        try:
            # Look for publication info
            pub_rows = first_edition.xpath('.//div[@class="dataRow"]')
            self.log.info('Found %d dataRow elements in first edition' % len(pub_rows))
            for row in pub_rows:
                title_text = row.xpath('.//div[@class="dataTitle"]/text()')
                value_text = row.xpath('.//div[@class="dataValue"]//text()')

                # Handle rows without dataTitle/dataValue (e.g., Published line)
                if not title_text or not value_text:
                    # Check if row text contains 'Published ... by ...' pattern
                    try:
                        row_text = row.text_content()
                    except AttributeError:
                        # lxml element doesn't have text_content, skip this row
                        continue
                    self.log.info('Editions page row with no dataTitle/dataValue: %r' % row_text[:200])
                    if row_text:
                        pub_match = re.search(r'Published\s+(.+?)\s+by\s+(.+)', row_text, re.IGNORECASE)
                        if pub_match:
                            mi.pubdate = pub_match.group(1).strip()
                            mi.publisher = pub_match.group(2).strip()
                            self.log.info('Parsed pubdate=%s publisher=%s from editions page' % (mi.pubdate, mi.publisher))
                    continue

                # Log what we found for debugging
                self.log.info('Editions page row: title_text=%r value_text=%r' % (title_text, value_text[:100] if value_text else None))

                title_text = title_text[0].strip().lower()
                value_text = ' '.join([v.strip() for v in value_text if v.strip()])

                if 'published' in title_text:
                    # Parse publication date and publisher
                    # Format: "Published May 14th 2013 by Mariner Books"
                    pub_match = re.search(r'Published\s+(.+?)\s+by\s+(.+)', value_text)
                    if pub_match:
                        mi.pubdate = pub_match.group(1).strip()
                        mi.publisher = pub_match.group(2).strip()
                    else:
                        mi.pubdate = value_text
                elif 'asin' in title_text:
                    # ASIN found
                    mi.asin = value_text
                elif 'publisher' in title_text:
                    mi.publisher = value_text
                elif 'isbn:' in title_text:
                    isbn = check_isbn(value_text)
                    if isbn:
                        mi.isbn = isbn
                elif 'average rating' in title_text:
                    rating = self.parse_rating_from_text(value_text)
                    if rating is not None:
                        mi.rating = rating
                elif 'ratings' in title_text or 'rating' in title_text:
                    votes = self.parse_votes_from_text(value_text)
            ...
            self.log.exception('Error parsing edition data: %s' % e)

        # Fallback to autocomplete data for missing fields
        if mi.rating is None and self.avg_rating is not None:
            try:
                self.log.info('Using autocomplete avg_rating as fallback: %s' % self.avg_rating)
                mi.rating = float(self.avg_rating)
            except (ValueError, TypeError):
                pass

        if mi.comments is None and self.description_html:
            try:
                # Filter out editions snippets which are not actual descriptions
                desc_text = self.description_html
                if desc_text and not desc_text.lower().startswith('<p>editions for') and not desc_text.lower().startswith('editions for'):
                    self.log.info('Using autocomplete description as fallback, length: %d' % len(desc_text))
                    mi.comments = sanitize_comments_html(desc_text)
                else:
                    self.log.info('Skipping autocomplete description - appears to be editions snippet')
            except Exception as e:
                self.log.exception('Failed to use autocomplete description')

        # Try to get description from meta tags or hidden elements
        try:
            # Check for description in meta tags
            desc_meta = root.xpath('//meta[@name="description"]/@content')
            if desc_meta:
                desc = desc_meta[0].strip()
                # Filter out editions snippets which are not actual descriptions
                if desc and len(desc) > 50 and not desc.lower().startswith('editions for'):
                    mi.comments = sanitize_comments_html(desc)
                else:
                    self.log.info('Skipping meta description - appears to be editions snippet')
            # Also check for twitter:description
            if not mi.comments:
                twitter_desc = root.xpath('//meta[@name="twitter:description"]/@content')
                if twitter_desc:
        ...
        # Try to get ASIN/ISBN from hidden shelf forms, which are often present on editions pages.
        try:
            shelf_form = first_edition.xpath('.//form[contains(@action, "shelf/add_to_shelf")]')
            if shelf_form:
                form = shelf_form[0]
                book_id_input = form.xpath('.//input[@name="book_id"]/@value')
                if book_id_input and not self.goodreads_id:
                    self.goodreads_id = book_id_input[0]
                    mi.set_identifier('goodreads', self.goodreads_id)
        except:
            pass

        # Get cover URL from the main page image
        try:
            img_node = root.xpath('//img[@class="bookCover"]')
            if img_node:
                img_src = img_node[0].get('src')
                if img_src:
                    # Convert to larger image
                    self.cover_url = img_src.replace('_SY75_', '_SX500_')
                    mi.has_cover = True
        except:
            pass
        ...
        # Fallback to autocomplete data for missing fields
        try:
            # Use autocomplete rating if we don't have one
            if mi.rating is None and self.avg_rating is not None:
                mi.rating = float(self.avg_rating)
                self.log.info('Using autocomplete rating as fallback: %s' % mi.rating)
            # Use autocomplete ratings count if we don't have one
            if 'grvotes' not in mi.identifiers and self.ratings_count is not None:
                mi.set_identifier('grvotes', str(self.ratings_count))
                self.log.info('Using autocomplete ratings count as fallback: %s' % self.ratings_count)
            # Use autocomplete rating as grrating identifier if not already set
            if 'grrating' not in mi.identifiers and self.avg_rating is not None:
                mi.set_identifier('grrating', str(self.avg_rating))
                self.log.info('Using autocomplete rating as grrating identifier: %s' % self.avg_rating)
            # Use autocomplete description if we don't have one and it's not an editions snippet
            if not mi.comments and self.description_html:
                # Filter out editions snippets which are not actual descriptions
                desc_text = self.description_html
                if desc_text and not desc_text.lower().startswith('<p>editions for') and not desc_text.lower().startswith('editions for'):
                    mi.comments = sanitize_comments_html(desc_text)
                    self.log.info('Using autocomplete description as fallback')
                else:
                    self.log.info('Skipping autocomplete description - appears to be editions snippet')
        except:
            pass
    ...
    def strip_edition_suffix(self, title):
        if not title:
            return title
        # Remove common Goodreads edition markers at the end of the title.
        title = self.plugin.EDITION_SUFFIX_RE.sub('', title).strip()
        return title

    def parse_rating_from_text(self, text):
        if not text:
            return None
    ...
    def parse_votes_from_text(self, text):
        if not text:
            return None
        match = re.search(r'\(([0-9,]+)\s+ratings?\)', text, flags=re.IGNORECASE)
        if match:
            try:
